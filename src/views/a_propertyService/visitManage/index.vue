    ...
    <div class="filter-container">
      ...
      <el-form :inline="true" :model="listQuery" class="demo-form-inline">
        <el-form-item label="访客姓名">
          <el-input v-model="listQuery.filter" placeholder="请输入访客姓名" />
        </el-form-item>
        <el-form-item>
      ...
      <el-table
        ...
        style="width: 100%"
      >
        <el-table-column label="访客姓名" prop="code" align="center">
          <template slot-scope="{ row }">
            <span>{{ row.code }}</span>
          </template>
        </el-table-column>
        <el-table-column label="身份证号" prop="type3" align="center">
          <template slot-scope="{ row }">
            <span>{{ row.code }}</span>
          </template>
        </el-table-column>
        <el-table-column label="来访事件" prop="type3" align="center">
          <template slot-scope="{ row }">
            <span>{{ row.code }}</span>
          </template>
        </el-table-column>
        <el-table-column label="预计离开时间" prop="type3" align="center">
          <template slot-scope="{ row }">
            <span>{{ row.code }}</span>
          </template>
        </el-table-column>
